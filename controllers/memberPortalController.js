...
export const getMyProfile = async (req, res) => {
  try {
    ...
    if (roleName === 'ondemanduser') {
      let targetGuestId = req.guestId || req.params?.guestId || req.params?.memberId || req.query?.guestId || req.query?.memberId;
      if (!targetGuestId && req.user) {
        ...
        guest = await Guest.findById(targetGuestId).populate('buildingId', 'name openingTime closingTime');
      }
      ...
      member = await Member.findById(targetMemberId || req.memberId)
        .populate({
          path: 'desk',
          populate: [
            { path: 'cabin', select: 'name number type' },
            { path: 'building', select: 'name address' }
          ]
        })
        .populate('client', 'name email membershipStatus');
    ...
    if (roleName === 'ondemanduser') {
      // For ondemanduser, ensure they are accessing their own guest profile (if user ref is available)
      if (req.user?._id && guest && String(guest._id) !== String(req.guestId)) {
        // If guestId in token doesn't match requested one
        // (Optional: extra security logic here)
      }
    } else if (isClient) {
      ...
      if (req.clientId && String(member.client?._id || '') !== String(req.clientId || '')) {
        return res.status(403).json({ success: false, message: `Access denied: member.client._id (${String(member.client?._id)}) does not match req.clientId (${String(req.clientId)})` });
      }
    }
    ...
    const profileData = roleName === 'ondemanduser' ? {
      _id: guest._id,
      name: guest.name,
  ...
  try {
    ...
    if (isClient) {
      if (req.client) {
        ...
        membershipStatus = (typeof req.client.membershipStatus === 'boolean') ? (req.client.membershipStatus ? 'active' : 'invoice pending') : (req.client.membershipStatus || "active");
      ...
      if (member) {
        name = `${member.firstName || ''} ${member.lastName || ''}`.trim();
        membershipStatus = (typeof member.client?.membershipStatus === 'boolean') ? (member.client.membershipStatus ? 'active' : 'invoice pending') : (member.client?.membershipStatus || "active");
  ...
  try {
    ...
    if (roleName === 'ondemanduser') {
      ...
      if (guest) {
        ...
        cabinNumber = dayPassCount ? String(dayPassCount) : "0";
        cabinType = "On demand";
    ...
    try {
      if (roleName === 'ondemanduser' && req.guestId) {
        ...
        if (bookingQuery || dayPassQuery) {
          const [rawBookings, dayPasses] = await Promise.all([
            bookingQuery ? MeetingBooking.find(bookingQuery)
              .populate({
                path: 'room',
                select: 'name images building amenities capacity pricing floor',
                populate: {
          ...
          if (dayPasses.length > 0) {
            ...
            commonAreas.forEach(ca => {
              ...
              if (!buildingImageMap[bId]) {
                const primaryImg = (ca.images || []).find(img => img.isPrimary) || (ca.images || [])[0];
                if (primaryImg?.url) buildingImageMap[bId] = primaryImg.url;
              }
            });
          }

          const mappedMeetings = rawBookings.map(b => {
            const bookingStart = new Date(b.start);
            const bookingEnd = new Date(b.end);
            const istYmd = bookingStart.toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' });
            const startTimeStr = bookingStart.toLocaleTimeString('en-US', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit', hour12: true }).toUpperCase();
          ...
          const mappedPasses = dayPasses.map(dp => {
            const istYmd = dp.visitDate ? new Date(dp.visitDate).toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' }) : null;
            return {
              _id: dp._id,
              bookingId: dp._id,
              building: dp.building?._id,
